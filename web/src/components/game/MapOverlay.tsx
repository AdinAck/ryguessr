...
export const MapOverlay = memo(
  ({
    ...
    const mapOptions = useMemo(() => {
      return {
        draggableCursor: hasGuessed ? "move" : "crosshair",
    ...
    `;
      ...
      return {
        ...
        anchor: window.google
          ? new window.google.maps.Point(15, 20)
          : undefined,
          ...
          >
            ...
            {hasGuessed && selectedLocation && roundData && (
              <>
                <Marker
                  position={{
                    lat: roundData.real_location.lat,
                    lng: roundData.real_location.lng,
                  }}
                ...
                <Polyline
                  ...
                  path={interpolateGreatCircle(
                    selectedLocation,
                    roundData.real_location,
                    100,
                  )}
                />
                {Object.values(roundData.player_results).map(
                  (entry: PlayerResults) => {
                    const isCurrentUser =
                      Math.abs(
                        entry.guess_location.lat - selectedLocation.lat,
                      ) < 0.00001 &&
                      Math.abs(
                        entry.guess_location.lng - selectedLocation.lng,
                      ) < 0.00001;

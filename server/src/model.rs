...
use crate::{
    Handle, Room, colors::PlayerColor, geo::Location, handle, name_gen::NameGenerator, room,
};

/// Methods on [`Model`] are synchronous, anything async should be in [`Context`](crate::Context).
#[derive(Default)]
pub struct Model {
...
impl Model {
    ...
    pub fn set_name(
        ...
        self.client_mut(client_id)?.username = new_username.clone();

        // Update the client's name in their current room.
        if let Some(member) = self.client_room_mut(client_id)?.members.get_mut(client_id) {
    ...
    pub fn set_color(
        ...
        self.client_mut(client_id)?.color = PlayerColor {
            srgb: new_color,
            custom: true,
        };

        // Update the member's color in their current room.
        let room = self.client_room_mut(client_id)?;
    ...
    pub fn client_mut(&mut self, client_id: &handle::Id) -> Result<&mut Handle, StatusCode> {
        self.clients
            .get_mut(client_id)
    ...
    pub fn drop_room_if_idle(&mut self, room_id: &room::Id) {
